...
@method_decorator(name='list',
    decorator=swagger_auto_schema(
        tags=['certifications'],
        operation_description="로그인한 사용자의 인증 리스트를 모두 가져옵니다.",
        manual_parameters=[
            openapi.Parameter(
                'Authorization', openapi.IN_HEADER,
                type=openapi.TYPE_STRING,
...
class GetCertificationInfoViewset(viewsets.GenericViewSet,
                                  mixins.ListModelMixin):
    authentication_classes = [JSONWebTokenAuthentication]
    permission_classes = [custom_permission.IsOwnerOrReadOnly]
    queryset = Certification.objects
...
@method_decorator(name='list',
    decorator=swagger_auto_schema(
        tags=['certifications'],
        operation_description="사용자가 좋아요 한 인증 리스트를 조회합니다.",
        manual_parameters=[
            openapi.Parameter(
                'Authorization', openapi.IN_HEADER,
                type=openapi.TYPE_STRING,
...
class LikedCertificationByUserViewset(viewsets.GenericViewSet,
                                    mixins.ListModelMixin):
    authentication_classes = [JSONWebTokenAuthentication]
    permission_classes = [custom_permission.IsOwnerOrReadOnly]
    queryset = Certification.objects
    ...
    def list(self, request):
        certification = get_user_liked_certification_by_owner(request.user)
